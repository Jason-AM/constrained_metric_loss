...
class LinearScore(nn.Module):
    ...
    def fit(self, x, y, optimizer, n_epochs=10_000):
...
class MLP(nn.Module):
    """Normal logistic regression with BCE loss"""
    def __init__(self, loss, loss_arguments, trainable_loss_arguments=False):
        super().__init__()
...
mlp_model = MLP(loss=nn.BCEWithLogitsLoss, loss_arguments={})
...
mlp_minprec_model = MLP(loss=MinPrecLoss, loss_arguments=loss_params)
