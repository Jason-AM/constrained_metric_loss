...
from constrained_metric_loss.min_precision_loss import MinPrecLoss, MinPrecLeakyLoss
...
class MLP(nn.Module):
    ...
    def fit(self, x, y, optimizer, n_epochs=200):
...
loss_params = {
    "min_prec": 0.8, 
    "lmbda": nn.Parameter(torch.tensor([1e2])), 
    'lmbda2': 0, # nn.Parameter(torch.tensor([1e2])), 
    'leaky_slope': 0.1
}

mlp_minprec_model = MLP(hidden_width=4, loss=MinPrecLeakyLoss, loss_arguments=loss_params)

mlp_minprec_model.apply(weights_init)


optimizer_minprec = optim.Adam(mlp_minprec_model.parameters(), lr=0.1, amsgrad=False)
losses = mlp_minprec_model.fit(xtrain_toy, ytrain_toy, optimizer_minprec, n_epochs=100)
...
decision_line_minprec = decisionLine(mlp_minprec_model, xtest_toy, thresh=0.5, eps=0.01, gridsize=100)
